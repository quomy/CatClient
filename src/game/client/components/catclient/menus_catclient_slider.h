...
namespace CatClientMenu
{
	...
	inline float DoRoundScrollbarH(CUi *pUi, const void *pId, const CUIRect &Rect, float Current)
	{
	...
	inline bool DoSliderOption(CUi *pUi, const void *pId, CSliderState *pState, int *pValue, const CUIRect &Rect, int Min, int Max, const IScrollbarScale *pScale, bool DelayApply, TFormatFn &&FormatValue)
	{
		...
		int NewValue = pScale->ToAbsolute(DoRoundScrollbarH(pUi, pId, ScrollBar, pScale->ToRelative(Value, Min, Max)), Min, Max);
