...
static void RenderStreamerSettingsSection(CMenus *pMenus, CUi *pUi, CUIRect &View)
{
	...
	pUi->DoLabel(&Label, Localize("Streamer Mode"), CATCLIENT_MENU_HEADLINE_FONT_SIZE, TEXTALIGN_ML);
	Content.HSplitTop(CATCLIENT_MENU_MARGIN_SMALL, nullptr, &Content);

	pMenus->DoButton_CheckBoxAutoVMarginAndSet(&g_Config.m_CcStreamerMode, Localize("Enable Streamer Mode"), &g_Config.m_CcStreamerMode, &Content, CATCLIENT_MENU_LINE_SIZE);
	Content.HSplitTop(CATCLIENT_MENU_MARGIN_SMALL, nullptr, &Content);
...
static EBlockedWordsAction RenderBlockedWordsSection(CMenus *pMenus, CUi *pUi, CCatClient *pCatClient, CUIRect &View)
{
	...
	if(!gs_ShowBlockedWordsList)
	{
		...
		pUi->DoLabel(&Description, "The list is hidden by default for stream safety.", CATCLIENT_MENU_SMALL_FONT_SIZE, TEXTALIGN_ML, Props);
	...
	if(vWords.empty())
	{
		...
		{
			pUi->DoLabel(&EmptyRow, "No blocked words yet", CATCLIENT_MENU_SMALL_FONT_SIZE, TEXTALIGN_MC);
		}
	}
	else
	{
