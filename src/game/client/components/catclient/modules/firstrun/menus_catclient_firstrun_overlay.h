#ifndef GAME_CLIENT_COMPONENTS_CATCLIENT_MODULES_FIRSTRUN_MENUS_CATCLIENT_FIRSTRUN_OVERLAY_H
#define GAME_CLIENT_COMPONENTS_CATCLIENT_MODULES_FIRSTRUN_MENUS_CATCLIENT_FIRSTRUN_OVERLAY_H
...
void CMenus::RenderFirstRunSetupOverlay(const CUIRect &Screen)
{
	...
	char aStepLabel[64];
	str_format(aStepLabel, sizeof(aStepLabel), "%s %d/%d", CCLocalize("Step"), m_FirstRunSetupStep + 1, (int)NUM_FIRST_RUN_SETUP_STEPS);
	...
	if(DoButton_Menu(&s_SkipButton, CCLocalize("Skip setup"), 0, &SkipButton))
	{
	...
	{
		if(DoButton_Menu(&s_BackButton, CCLocalize("Back"), 0, &BackButton))
		{
			m_FirstRunSetupStep--;
	...
	else
	{
		...
		Ui()->DoLabel(&BackButton, CCLocalize("Back"), 12.0f, TEXTALIGN_MC);
	}

	static CButtonContainer s_NextButton;
	const bool LastStep = m_FirstRunSetupStep == NUM_FIRST_RUN_SETUP_STEPS - 1;
	if(DoButton_Menu(&s_NextButton, LastStep ? CCLocalize("Finish") : CCLocalize("Next"), 0, &NextButton))
	{
		if(LastStep)
